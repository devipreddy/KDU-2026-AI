...
def process_ingestion_job(
    ...
    *,
    ...
    try:
        ...
        review_gate = {
            "required": not chunk_after_extraction,
            "status": "pending" if not chunk_after_extraction else "not_required",
        ...
        document.status = (
            DocumentStatus.PROCESSED if chunk_after_extraction else DocumentStatus.REVIEW_REQUIRED
        )
        job.status = IngestionJobStatus.SUCCEEDED
        job.stage = "text_extracted" if chunk_after_extraction else "text_review_required"
        ...
        db.commit()
        if chunk_after_extraction and should_index_after_extraction(index_after_extraction):
