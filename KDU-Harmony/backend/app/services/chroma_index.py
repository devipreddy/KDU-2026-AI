...
def get_chroma_client() -> Any:
    return get_cached_chroma_client(
        str(settings.chroma_host) if settings.chroma_host else "",
        str(settings.chroma_persist_path.resolve()),
    )


@lru_cache(maxsize=4)
def get_cached_chroma_client(chroma_host: str, persist_path: str) -> Any:
    try:
        import chromadb
...
def ensure_chroma_collection(
    ...
    if client is None:
        return get_cached_chroma_collection(
            str(settings.chroma_host) if settings.chroma_host else "",
            str(settings.chroma_persist_path.resolve()),
            mapping.collection_name,
            json.dumps(mapping.to_collection_metadata(), sort_keys=True),
        )
...
def get_cached_chroma_collection(
    ...
    client = get_cached_chroma_client(chroma_host, persist_path)
    return client.get_or_create_collection(
        name=collection_name,
        metadata=json.loads(collection_metadata_json),
    )
