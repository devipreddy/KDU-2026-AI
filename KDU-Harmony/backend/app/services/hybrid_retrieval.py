...
HYBRID_OVERLAP_BOOST = 0.004
BM25_EXACT_QUALITY_BOOST_FACTOR = 0.0005
BM25_EXACT_QUALITY_BOOST_CAP = 0.006
...
def hybrid_search(
    ...
    *,
    ...
    authorization = build_authorized_metadata_filter(
        db,
        user=user,
        query=parsed_query,
...
def add_ranked_hits(
    ...
    *,
    ...
    for rank, hit in enumerate(hits, start=1):
        ...
        accumulator["quality_boost"] += retrieval_quality_boost(retriever, hit)
...
def retrieval_quality_boost(
    retriever: str,
    hit: BM25SearchHit | DenseSearchHit,
) -> float:
    if retriever != "bm25" or not isinstance(hit, BM25SearchHit):
        return 0.0
    return min(
        max(hit.exact_match_score, 0.0) * BM25_EXACT_QUALITY_BOOST_FACTOR,
        BM25_EXACT_QUALITY_BOOST_CAP,
    )
...
def build_hybrid_hit(accumulator: dict[str, Any]) -> HybridSearchHit:
    ...
    overlap_boost = HYBRID_OVERLAP_BOOST if bm25_hit is not None and dense_hit is not None else 0.0
    final_score = accumulator["score"] + accumulator["quality_boost"] + overlap_boost
    return HybridSearchHit(
        ...
        source_metadata=source_metadata(
            bm25_hit=bm25_hit,
            dense_hit=dense_hit,
            source_overlap_boost=overlap_boost,
            bm25_exact_quality_boost=accumulator["quality_boost"],
        ),
    )
...
def source_metadata(
    *,
    ...
    source_overlap_boost: float = 0.0,
    bm25_exact_quality_boost: float = 0.0,
) -> dict[str, Any]:
    metadata: dict[str, Any] = {
        "fusion": {
            "source_overlap_boost": source_overlap_boost,
            "bm25_exact_quality_boost": round(bm25_exact_quality_boost, 8),
        }
    }
