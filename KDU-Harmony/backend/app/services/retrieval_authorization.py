...
from app.services.care_access import organization_scopes_for_user
...
def build_authorized_metadata_filter(
    ...
    *,
    ...
    organization_scopes = organization_scopes_for_user(db, user)
    if policies_require_organization_scope(policies) and not organization_scopes:
        return deny_all_filter(
            user=user,
            role_names=role_names,
            query_filters=query_filters,
            policy_names=[policy.policy_name for policy in policies],
            deny_reason="no_active_organization_scope",
        )
...
def build_authorization_filter_payload(
    *,
    ...
    organization_scopes: list[str] | None = None,
) -> dict[str, Any]:
    ...
    scoped_hospitals = scoped_policy_values(
        policies=policies,
        condition_key="hospitals",
        runtime_values=organization_scopes,
    )
    if scoped_hospitals:
        filters["hospital"] = scoped_hospitals
...
def scoped_policy_values(
    *,
    policies: list[DocumentAccessPolicy],
    condition_key: str,
    runtime_values: list[str] | None,
) -> list[str]:
    policy_values = intersect_policy_values(policies, condition_key)
    runtime_values = normalize_string_list(runtime_values)
    if policy_values and runtime_values:
        scoped_values = sorted(set(policy_values).intersection(runtime_values))
        return scoped_values or ["__unauthorized__"]
    if policy_values:
...
def policies_require_organization_scope(policies: list[DocumentAccessPolicy]) -> bool:
    return any(
        policy.effect == AccessPolicyEffect.ALLOW.value
        and bool(policy.conditions.get("requires_organization_scope"))
        for policy in policies
    )
