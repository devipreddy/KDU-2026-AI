...
from app.services.care_access import (
    assigned_patient_refs_for_user,
    user_requires_patient_assignment_for_phi,
)
...
def render_contextual_search_result(
    ...
    *,
    ...
    assigned_patient_refs = assigned_patient_refs_for_user(db, user)
    assignment_required_for_phi = user_requires_patient_assignment_for_phi(user)
    decrypted_values = decrypted_values_by_token(
        mappings=mappings,
        policy=policy,
        assigned_patient_refs=assigned_patient_refs,
        assignment_required_for_phi=assignment_required_for_phi,
    )
    rendered_hits = [
        render_hit(
            hit,
...
def render_hit(
    ...
    *,
    ...
    assigned_patient_refs: set[str] | None = None,
    assignment_required_for_phi: bool = False,
) -> PHIRenderedSearchHit:
    redactions: set[str] = set()
    sensitive_terms = known_sensitive_terms_for_citation(hit.citation)
    assigned_patient_refs = assigned_patient_refs or set()
    effective_policy = rendering_policy_for_hit(
        policy,
        patient_ref=hit.citation.patient_ref,
        assigned_patient_refs=assigned_patient_refs,
        assignment_required_for_phi=assignment_required_for_phi,
    )
    matched_chunk = render_chunk(
        ...
        known_sensitive_terms=sensitive_terms,
    )
    parent_context = (
        render_chunk(
            hit.parent_context,
    ...
    retrieval["rendering"] = hit_rendering_metadata(
        base_policy=policy,
...
def rendering_policy_for_hit(
    policy: PHIRenderingPolicy,
    *,
    patient_ref: str | None,
    assigned_patient_refs: set[str],
    assignment_required_for_phi: bool,
) -> PHIRenderingPolicy:
    if (
        not assignment_required_for_phi
        or policy.render_mode != "full_phi"
        or (patient_ref and patient_ref in assigned_patient_refs)
    ):
        return policy
    return PHIRenderingPolicy(
        ...
        render_mode="de_identified",
        can_decrypt_phi=False,
        include_clinical_text=policy.include_clinical_text,
        allowed_entity_types=[],
    )


def hit_rendering_metadata(
    *,
    base_policy: PHIRenderingPolicy,
    effective_policy: PHIRenderingPolicy,
    patient_ref: str | None,
    assigned_patient_refs: set[str],
    assignment_required_for_phi: bool,
) -> dict[str, Any]:
    if not assignment_required_for_phi or base_policy.render_mode != "full_phi":
        patient_assignment = "not_required"
    elif patient_ref and patient_ref in assigned_patient_refs:
        patient_assignment = "assigned"
    else:
        patient_assignment = "unassigned_generalized"
    return {
        "render_mode": effective_policy.render_mode,
        "phi_visibility": effective_policy.phi_visibility,
...
def render_chunk(
    ...
    *,
    ...
    decrypted_values: dict[str, str],
    redactions: set[str],
    known_sensitive_terms: dict[str, str] | None = None,
) -> RenderedChunk:
    text = (
        render_text(
            chunk.text,
    ...
    text = mask_known_sensitive_terms(
        text,
        terms=known_sensitive_terms or {},
        policy=policy,
        redactions=redactions,
    )
...
def render_citation(
    ...
    *,
    ...
    return RenderedCitation(
        ...
        hospital=render_sensitive_metadata_text(
            citation.hospital,
            entity_type="FACILITY_NAME",
            policy=policy,
            redactions=redactions,
        ),
        physician=render_sensitive_metadata_text(
            citation.physician,
            entity_type="PROVIDER_NAME",
            policy=policy,
            redactions=redactions,
        ),
...
def known_sensitive_terms_for_citation(citation: SourceCitation) -> dict[str, str]:
    terms: dict[str, str] = {}
    if citation.hospital:
        terms[citation.hospital] = "FACILITY_NAME"
    if citation.physician:
        terms[citation.physician] = "PROVIDER_NAME"
    return terms


def mask_known_sensitive_terms(
    text: str | None,
    *,
    terms: dict[str, str],
    policy: PHIRenderingPolicy,
    ...
    if text is None or policy.render_mode == "full_phi":
        return text
    rendered = text
    for term, entity_type in sorted(terms.items(), key=lambda item: len(item[0]), reverse=True):
        if not term:
            continue
...
def render_sensitive_metadata_text(
    value: str | None,
    *,
    entity_type: str,
    policy: PHIRenderingPolicy,
    ...
    if policy.render_mode == "full_phi":
        return value
    redactions.add(entity_type)
    return placeholder_for_entity(entity_type, mode=policy.render_mode)
...
def entity_type_for_metadata_key(key: str) -> str:
    ...
    if normalized in {"facilities", "facility", "hospital", "hospital_names", "hospitals"}:
...
def patient_display_ref(
    ...
    *,
    ...
    if policy.render_mode == "full_phi":
        if patient_ref:
            patient_token = f"[{patient_ref}]"
            mapping = mappings.get(patient_token)
            if (
                policy.can_decrypt_phi
                and mapping is not None
                and mapping.entity_type == "PATIENT_NAME"
                and mapping.entity_type in set(policy.allowed_entity_types)
            ):
                return decrypted_values.get(patient_token, patient_ref)
    ...
    digest = hashlib.sha256(stable_ref.encode()).hexdigest()[:10].upper()
    ...
    *,
    ...
    for token, mapping in mappings.items():
        if mapping.entity_type not in allowed:
            continue
        if assignment_required_for_phi and mapping.patient_ref not in assigned_patient_refs:
            continue
...
def tokens_in_context_result(context_result: ContextualSearchResult) -> set[str]:
    tokens: set[str] = set()
    for hit in context_result.hits:
        if hit.citation.patient_ref:
            tokens.add(f"[{hit.citation.patient_ref}]")
