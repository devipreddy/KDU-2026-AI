...
def exact_match_score(matched_fields: dict[str, list[str]]) -> float:
    ...
    for field_name, terms in matched_fields.items():
        field_boost = MRN_EXACT_BOOST if field_name == "mrn_tokens" else EXACT_FIELD_BOOST
...
def tokenize_lexical_text(text: str) -> list[str]:
    return [normalize_token(match.group(0)) for match in TOKEN_PATTERN.finditer(text)]
...
def contains_exact_term(text: str, term: str) -> bool:
    normalized_text = " ".join(tokenize_lexical_text(text))
    normalized_term = " ".join(tokenize_lexical_text(term))
    return bool(
        normalized_term
        and re.search(rf"(?<!\S){re.escape(normalized_term)}(?!\S)", normalized_text)
    )
