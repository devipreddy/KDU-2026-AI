...
The backend now includes authentication, ingestion, OpenRouter Qianfan OCR, PHI handling, ChromaDB retrieval,
cross-encoder reranking, LLM answer synthesis, audit logging, observability, timelines, and
benchmarks.
...
The pipeline decrypts the local storage object, verifies the original SHA-256 checksum, extracts UTF-8 text or typed PDF text with PyMuPDF, writes processed text under `../data/processed`, and marks the ingestion job as succeeded. OCR-required documents remain queued for the OpenRouter Qianfan OCR worker.
...
When `presidio-analyzer` is installed and `PRESIDIO_ENABLED=true`, Microsoft Presidio findings are
merged with the local rule-based recognizers before tokenization; otherwise the rule-based detector
...
allowed sensitivity levels and organization scopes, so unauthorized chunks are excluded before
vector or keyword retrieval. Seeded clinicians are scoped to `Harmony General Hospital`; a doctor
query for another hospital is denied before BM25 or dense retrieval runs.
...
```powershell
.\.venv\Scripts\python.exe -m app.services.cross_encoder_reranking doctor@example.com "patients with cardiac issues treated in Q1 2025" --rerank-top-n 8
```

The reranker uses `BAAI/bge-reranker-base` by default for local demo latency and falls back to
`cross-encoder/ms-marco-MiniLM-L-6-v2` if the preferred model cannot be loaded. Only the top-N
hybrid candidates are scored by the cross-encoder to keep latency bounded.
...
without clinical snippets or parent section text. Doctor PHI is also patient-assignment aware:
seeded `doctor@example.com` only sees full identifiers for assigned Harmony patients such as
`PATIENT_REF_0050`, `PATIENT_REF_0201`, `PATIENT_REF_0012`, and `PATIENT_REF_0229`; unassigned
same-organization results render as generalized/de-identified clinical context.

## Live Search API And LLM Answering

Run the same pipeline used by the frontend:
...
The UI uses the streaming variant at `POST /api/v1/search/stream`. It sends a retrieval event as
soon as BM25, dense retrieval, fusion, reranking, context expansion, rendering, and audit logging
finish, then streams LLM answer deltas over server-sent events before sending the final response.

The API performs query understanding, RBAC metadata filtering, BM25, ChromaDB dense retrieval,
reciprocal-rank fusion, cross-encoder reranking, parent context expansion, PHI-aware rendering,
audit logging, and evidence-grounded answer generation. Set `OPENAI_API_KEY` in `backend/.env` to
call the OpenAI Responses API. If the key is missing, retrieval still returns and the answer status
is `not_configured`.
...
LangSmith tracing wraps the live search API, PHI-aware retrieval pipeline, and LLM answer generation
with PHI-redacted inputs, metadata, and outputs when `LANGSMITH_TRACING=true`.
...
allows PHI visibility and, for doctors, only when the token belongs to an assigned patient. Each
allowed, denied, or not-found lookup writes a `phi_decrypt` audit event.

## OCR Pipeline

Process queued scanned PDF ingestion jobs with OpenRouter's Qianfan OCR model:

```powershell
.\.venv\Scripts\python.exe -m app.services.document_ocr
```

The OCR worker renders PDF pages to PNG images, sends each page to
`baidu/qianfan-ocr-fast`, stores model metadata and explicit API errors, writes processed text,
and routes review-gated extraction through the ingestion dashboard. If OpenRouter reports that the
primary model has no endpoint, the worker tries `OPENROUTER_OCR_FALLBACK_MODELS` in order before
surfacing the combined provider error.
Multi-page PDFs are processed with a small bounded page-level worker pool. Set
`OPENROUTER_OCR_MAX_WORKERS=1` if your OpenRouter tier rate-limits parallel page requests, or raise
it cautiously for faster multi-page OCR.

Configure OpenRouter before running OCR:

```powershell
OPENROUTER_API_KEY=your_openrouter_key
OPENROUTER_OCR_MODEL=baidu/qianfan-ocr-fast
OPENROUTER_OCR_FALLBACK_MODELS=baidu/qianfan-ocr-fast:free
OPENROUTER_OCR_MAX_WORKERS=2
```

To inspect the exact system prompt sent to Qianfan:

```powershell
.\.venv\Scripts\python.exe -m app.services.document_ocr --show-system-prompt
```
...
Load the synthetic records into Postgres as searchable documents and chunks:

```powershell
.\.venv\Scripts\python.exe -m app.synthetic.load_dataset
```

After loading, BM25 and hybrid retrieval have real document chunks to search. Dense vector results
require embedding/indexing:

```powershell
.\.venv\Scripts\python.exe -m app.services.embedding_pipeline --limit 500
```

## Latency Notes

The backend keeps hot retrieval resources in process memory: Chroma clients/collections are reused,
BGE embedding and cross-encoder model loads are single-flight cached, and the Presidio analyzer is
initialized once per process instead of once per document. First search after a cold backend start can
still be slower while local models load; subsequent searches should avoid that startup cost.
